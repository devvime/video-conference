...
function addVideo(labelMiniVidId, username, stream) {
  ...
  if (labelMiniVidId === 'my-label-mini-vid') {
    video.muted = true;
  }
...
function playStop() {
  ...
  if (enabled) {
    ...
    if (myVideoStream1) myVideoStream1.getVideoTracks()[0].enabled = false;
  } else {
    const html = `
...
function actions() {
  ...
  $('#sendMessage').click(() => {
    ...
    if (uploadState == 0) {
      ...
      uploadFile();
      const html = '<a href="uploads/' + text.val() + '" target="_blank">' + text.val() + '</a>';
...
<div id="right-chatroom">
 ...
 <p id="time-text" style="font-size:11px; color:#303030; margin-left: 55px; margin-top: 0px; padding-top: 0px; margin-bottom: 12px;">${time}</p>
 `)
  scrollToBottom();
})

socket.on('user-leave', (peerId, peerName) => {
  alert(peerName + " saiu da meet");
